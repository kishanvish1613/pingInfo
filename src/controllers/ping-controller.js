...
//         await Ping.updateOne({ host }, { status: 'active' });
...
const stopPingMonitoring = async (req, res) => {
    const host = req.body.host;
    console.log(`Received request to stop monitoring for host: ${host}`); // Debugging log
    ...
    if (intervalIds[host]) {
        ...
        console.log(`Stopped monitoring for host: ${host}`); // Debugging log

        req.flash('success', `Ping monitoring stopped for ${host}.`);
    } else {
        console.log(`No monitoring found for host: ${host}`); // Debugging log
        req.flash('error', `Ping monitoring is not running for ${host}.`);
    }
